      ...
      <div className="page-header" style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: "1rem", marginBottom: "0.5rem" }}>
            <Link href="/vendors" style={{ color: "#64748b", textDecoration: "none", fontSize: "0.9rem" }}>← Back to Vendors</Link>
          </div>
          <h1 className="page-title">{vendor.companyName} Fulfillment History</h1>
          <div style={{ display: "flex", gap: "1.5rem", color: "#64748b", fontSize: "0.9rem", marginTop: "0.5rem" }}>
            {vendor.contactPerson && <span style={{ display: "flex", alignItems: "center", gap: "6px" }}><User size={14} /> {vendor.contactPerson}</span>}
            {vendor.email && <span style={{ display: "flex", alignItems: "center", gap: "6px" }}><Mail size={14} /> {vendor.email}</span>}
            {vendor.phone && <span style={{ display: "flex", alignItems: "center", gap: "6px" }}><Phone size={14} /> {vendor.phone}</span>}
          </div>
        </div>
      </div>
              ...
              <div key={deal.id} style={{ padding: "1.25rem", background: "#f8fafc", borderRadius: "12px", border: "1px solid var(--surface-border)", display: "flex", flexWrap: "wrap", gap: "1.5rem", alignItems: "center", justifyContent: "space-between" }}>
                ...
                <div style={{ flex: "1 1 300px" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "0.5rem" }}>
                    ...
                    <span style={{ color: "#94a3b8" }}>→ Assigned from Customer:</span>
                    <Link href={`/customers/${deal.customer.id}`} style={{ fontWeight: "700", color: "var(--primary)", textDecoration: "none" }}>
